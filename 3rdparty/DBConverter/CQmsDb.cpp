...
#define DB_QMS_VERSION 1

CQmsDb::CQmsDb(const QString &filename, CMainWindow *parent)
    : QObject(parent)
    , gui(parent)
{
    if(QFile::exists(filename))
    {
        gui->stdErr(tr("remove existing %1").arg(filename));
...
void CQmsDb::initDB()
{
    ...
    if(query.exec( "CREATE TABLE versioninfo ( version TEXT )"))
    {
        ...
        query.bindValue(":version", DB_QMS_VERSION);
        QUERY_EXEC(; );
    }
...
void CQmsDb::migrateDB(int version)
{
    QSqlQuery query(db);
