    ...
    if(xmlSml.namedItem("DeviceLog").isElement())
    {
        ...
        if(xmlDeviceLog.namedItem("Header").isElement())
        {
            ...
            if(xmlHeader.namedItem("DateTime").isElement())
            {
                ...
                IUnit::parseTimestamp(dateTimeStr, time0); // local time is saved in case no UTC time is found in the samples
            }
            ...
            if (xmlSampleList.count() > 0)
            {
                ...
                { //look for samples with UTC timestamp
                    ...
                    if (xmlSample.namedItem("UTC").isElement())
                    {
                        ...
                        if (timeStr.indexOf("Z") != NOIDX)
                        { // UTC timestamp ; note the even this element is <UTC>, this does not mean that time is expressed as UTC
                            if(xmlSample.namedItem("Time").isElement())
                            {
                                IUnit::parseTimestamp(timeStr, time0);
                                time0 = time0.addMSecs(-xmlSample.namedItem("Time").toElement().text().toDouble() * 1000.0 );
                                UTCtimeFound = true;
                                break;
                            }
                        }
                ...
                if ( !UTCtimeFound)
                {
                    QMessageBox::warning(CMainWindow::getBestWidgetForParent(), tr("Use of local time...")
                                                   , tr("No UTC time has been found. "
                                                       "Local computer time will be used. "
                                                        "You can adjust the time using a time filter if needed.")
                                                   , QMessageBox::Ok);
                }
