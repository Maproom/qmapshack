...
void CKnownExtension::initMioTPX(IUnit &units)
{
    ...
    knownExtensions.insert("speed",
                           { QObject::tr("Speed"), 0., 600., units.speedfactor, units.speedunit, "://icons/32x32/CSrcSpeed.png", true, false,
                             getExtensionValueFunc("speed")});

    knownExtensions.insert("acceleration",
                           { QObject::tr("Acceleration"), std::numeric_limits<qreal>::lowest(), std::numeric_limits<qreal>::max(), units.basefactor, units.baseunit + "/s²", "://icons/32x32/CSrcAccel.png", true, false,
                             getExtensionValueFunc("acceleration")});

    knownExtensions.insert("course",
                           { QObject::tr("Course"), -3.2, 3.2, 1., "rad", "://icons/32x32/CSrcCourse.png", true, false,
...
void CKnownExtension::init(IUnit &units)
{
    ...
    {
        {internalSlope,
         { QObject::tr("Slope*"), -90., 90., 1., "°", "://icons/32x32/CSrcSlope.png", true, true,
           [](const CGisItemTrk::trkpt_t &p) { return p.slope1; }}
        },

        {internalSpeed,
         { QObject::tr("Speed*"), 0., 600., units.speedfactor, units.speedunit, "://icons/32x32/CSrcSpeed.png", true, true,
           [](const CGisItemTrk::trkpt_t &p) { return p.speed; }}
        },

        {internalEle,
         { QObject::tr("Elevation"), 0., 100000., units.basefactor, units.baseunit, "://icons/32x32/CSrcElevation.png", true, true,
           [](const CGisItemTrk::trkpt_t &p) { return (NOINT == p.ele) ? NOFLOAT : p.ele; }}
        },

        {internalProgress,
         { QObject::tr("Progress"), 0., NOFLOAT, units.basefactor, units.baseunit, "://icons/32x32/Progress.png", true, true,
...
{
    CKnownExtension def("", -100000., 100000., 1., "", "://icons/32x32/CSrcUnknown.png", false, true,
