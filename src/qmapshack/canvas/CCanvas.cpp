...
void CCanvas::findPoiCloseBy(const QPoint& px, QSet<CPoiItem>& poiItems, QList<QPointF>& posPoiHighlight)  const
{
    poi->findPoiCloseBy(px, poiItems, posPoiHighlight);

    CPoiItem mapPoi = map->findPOICloseBy(px);
...
void CCanvas::findPoisIn(const QRectF& degRect, QSet<CPoiItem>& poiItems, QList<QPointF>& posPoiHighlight) const
{
