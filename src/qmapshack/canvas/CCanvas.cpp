...
void CCanvas::findPoiCloseBy(const QPoint& px, QSet<const CPoiItem*>& poiItems, QList<QPointF>& posPoiHighlight)  const
{
    poi->findPoiCloseBy(px, poiItems, posPoiHighlight);

    const CPoiItem* mapPoi = map->findPOICloseBy(px);
    if(mapPoi != nullptr)
    {
...
void CCanvas::findPoisIn(const QRectF& degRect, QSet<const CPoiItem*>& poiItems, QList<QPointF>& posPoiHighlight) const
{
