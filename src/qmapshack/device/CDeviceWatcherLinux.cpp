...
QString CDeviceWatcherLinux::readMountPoint(const QString& path) {
  ...
  for (const QVariant& arg : reply.arguments()) {
    ...
    // This translates to an array of QByteArray. Don't konw why there
    // should be several mount points and if that is ever happening.
    // But we do not expect less wackyness from DBus definitions.
    ...
    while (!dbusArg.atEnd()) {
      ...
      mountPoints << decoder(data.removeLast());
    }
    dbusArg.endArray();
  }
#endif
