...
void CSearchDatabase::slotSearch() {
  ...
  while (query.next()) {
    ...
    QSqlQuery query2(db);
    query2.prepare(
        "SELECT t1.id, t1.type FROM folders AS t1 WHERE id=(SELECT parent FROM folder2item WHERE child=:id)");
...
void CSearchDatabase::addWithParentFolders(QTreeWidget* result, IDBFolder* child, QMap<quint64, IDBFolder*>& folders,
                                           QSqlDatabase& db) {
  QSqlQuery query(db);
