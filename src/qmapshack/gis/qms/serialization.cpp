...
QDataStream& operator>>(QDataStream& stream, CEnergyCycling::energy_set_t& e) {
  quint8 version;
  stream >> version >> e.driverWeight >> e.bikeWeight >> e.airDensity >> e.windSpeedIndex >> e.windSpeed >>
      e.windPositionIndex >> e.frontalArea >> e.windDragCoeff >> e.groundIndex >> e.rollingCoeff >> e.pedalCadence >>
      e.energyKcal;
  return stream;
}
...
QDataStream& CGisItemWpt::operator<<(QDataStream& stream) {
  ...
  if (version <= 2 && geocache.hasData) {
    // If the geocache was saved with an old Version of QMS recalculate it's key to make sure geocaches with the same id
