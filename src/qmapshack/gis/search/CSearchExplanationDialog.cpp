...
CSearchExplanationDialog::CSearchExplanationDialog(QWidget* parent) {
  ...
  QString explanation =
      tr("<p>The underlying syntax of the search is 'property comparison value', e.g. 'name contains water'. "
         "However some rules apply to make a search more intuitive.</p>");
  explanation +=
      tr("<p>The searches are always exclusive, e.g. searching for a track 'elevation under 500m' will find "
  ...
  explanation += "<li>";
  explanation +=
      tr("If a search string in the short form 'comparison value' is used "
  ...
  explanation += "<li>";
  explanation +=
      tr("'Date equals' matches everything that is within the next 24h, if time is provided with date, if not, then "
         "everything on the day queried. Be aware that that the times are converted to UTC before comparison and you "
         "local timezone is taken for the query.");
  explanation += "</li>";
  explanation += "<li>";
  explanation +=
      tr("If you enter no unit the default unit (what you see when viewing the property of the item) is used.");
  explanation += "</li>";
  explanation += "</ul>";
