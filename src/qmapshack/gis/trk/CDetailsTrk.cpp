...
CDetailsTrk::CDetailsTrk(CGisItemTrk& trk) : INotifyTrk(CGisItemTrk::eVisualDetails), trk(trk) {
  ...
  labelHelpTrackPointInfo->setText(
      tr("<b>Track Point Information</b><br/>"
  ...
  connect(spinLineWidth, static_cast<void (QSpinBox::*)(int)>(&QSpinBox::valueChanged), this,
          &CDetailsTrk::slotLineWidth);
  connect(checkWithArrows, &QCheckBox::toggled, this, &CDetailsTrk::slotWithArrows);

  setupStyleLimits(trk.colorSourceLimit, toolLimitAutoStyle, toolLimitUsrStyle, toolLimitSysStyle, spinLimitLow,
                   spinLimitHigh);
  ...
  connect(comboColor, static_cast<void (QComboBox::*)(int)>(&QComboBox::currentIndexChanged), this,
          &CDetailsTrk::slotColorChanged);
  connect(comboColorSource, static_cast<void (QComboBox::*)(int)>(&QComboBox::currentIndexChanged), this,
          &CDetailsTrk::slotColorSourceChanged);
  connect(comboGraph2, static_cast<void (QComboBox::*)(int)>(&QComboBox::currentIndexChanged), this,
          &CDetailsTrk::slotSetupGraph);
  connect(comboGraph3, static_cast<void (QComboBox::*)(int)>(&QComboBox::currentIndexChanged), this,
          &CDetailsTrk::slotSetupGraph);

  setupGraphLimits(trk.limitsGraph1, toolLimitAutoGraph1, toolLimitUsrGraph1, toolLimitSysGraph1, spinMinGraph1,
                   spinMaxGraph1);
  setupGraphLimits(trk.limitsGraph2, toolLimitAutoGraph2, toolLimitUsrGraph2, toolLimitSysGraph2, spinMinGraph2,
                   spinMaxGraph2);
  setupGraphLimits(trk.limitsGraph3, toolLimitAutoGraph3, toolLimitUsrGraph3, toolLimitSysGraph3, spinMinGraph3,
                   spinMaxGraph3);
...
void CDetailsTrk::setupGraphLimits(CLimit& limit, QToolButton* toolLimitAutoGraph, QToolButton* toolLimitUsrGraph,
                                   QToolButton* toolLimitSysGraph, QDoubleSpinBox* spinMinGraph,
                                   QDoubleSpinBox* spinMaxGraph) {
  ...
  auto limitAutoFunc = [this, &limit, spinMinGraph, spinMaxGraph](bool checked) {
    slotSetLimitModeGraph(CLimit::eModeAuto, &limit, spinMinGraph, spinMaxGraph, checked);
  };
  auto limitUserFunc = [this, &limit, spinMinGraph, spinMaxGraph](bool checked) {
    slotSetLimitModeGraph(CLimit::eModeUser, &limit, spinMinGraph, spinMaxGraph, checked);
  };
  auto limitSysFunc = [this, &limit, spinMinGraph, spinMaxGraph](bool checked) {
    slotSetLimitModeGraph(CLimit::eModeSys, &limit, spinMinGraph, spinMaxGraph, checked);
  };
  ...
  connect(spinMinGraph, static_cast<void (QDoubleSpinBox::*)(double)>(&QDoubleSpinBox::valueChanged), &limit,
          &CLimit::setMin);
  connect(spinMaxGraph, static_cast<void (QDoubleSpinBox::*)(double)>(&QDoubleSpinBox::valueChanged), &limit,
          &CLimit::setMax);

  auto limitChangedFunc = [this, &limit, spinMinGraph, spinMaxGraph]() {
    setupLimits(&limit, spinMinGraph, spinMaxGraph);
  };
  connect(&limit, &CLimit::sigChanged, this, limitChangedFunc);
}

void CDetailsTrk::setupStyleLimits(CLimit& limit, QToolButton* toolLimitAuto, QToolButton* toolLimitUsr,
                                   QToolButton* toolLimitSys, CDoubleSpinBox* spinMin, CDoubleSpinBox* spinMax) {
  bool isAutoMode = (limit.getMode() == CLimit::eModeAuto);
  ...
  connect(toolLimitAuto, &QToolButton::toggled, this,
          [this](bool checked) { slotSetLimitModeStyle(CLimit::eModeAuto, checked); });
  connect(toolLimitUsr, &QToolButton::toggled, this,
          [this](bool checked) { slotSetLimitModeStyle(CLimit::eModeUser, checked); });
  connect(toolLimitSys, &QToolButton::toggled, this,
          [this](bool checked) { slotSetLimitModeStyle(CLimit::eModeSys, checked); });
  ...
  connect(&limit, &CLimit::sigChanged, this,
          [this, &limit, spinMin, spinMax] { setupLimits(&limit, spinMin, spinMax); });
}
...
void CDetailsTrk::updateData() {
  ...
  CFilterChangeStartPoint* filterChangeStartPoint =
      tabWidget->findChild<CFilterChangeStartPoint*>("IFilterChangeStartPoint");
...
void CDetailsTrk::slotSetActivities() { CActivityTrk::getMenu(trk.getKey(), this, true); }
...
void CDetailsTrk::slotSetLimitModeGraph(CLimit::mode_e mode, CLimit* limit, QDoubleSpinBox* spinMin,
                                        QDoubleSpinBox* spinMax, bool on) {
  if (!on) {
    return;
  }
...
void CDetailsTrk::slotLineWidth(int f) { trk.lineScale = f; }
...
void CDetailsTrk::slotWithArrows(bool yes) { trk.showArrows = yes; }
