...
CDetailsWpt::CDetailsWpt(CGisItemWpt& wpt, QWidget* parent) : QDialog(parent), wpt(wpt) {
  ...
  connect(labelPosition, &QLabel::linkActivated, this,
          static_cast<void (CDetailsWpt::*)(const QString&)>(&CDetailsWpt::slotLinkActivated));
  connect(labelElevation, &QLabel::linkActivated, this,
          static_cast<void (CDetailsWpt::*)(const QString&)>(&CDetailsWpt::slotLinkActivated));
  connect(labelProximity, &QLabel::linkActivated, this,
          static_cast<void (CDetailsWpt::*)(const QString&)>(&CDetailsWpt::slotLinkActivated));
  connect(labelTime, &QLabel::linkActivated, this,
          static_cast<void (CDetailsWpt::*)(const QString&)>(&CDetailsWpt::slotLinkActivated));
  connect(textCmtDesc, &QTextBrowser::anchorClicked, this,
          static_cast<void (CDetailsWpt::*)(const QUrl&)>(&CDetailsWpt::slotLinkActivated));
...
void CDetailsWpt::setupGui() {
  ...
  if (wpt.getTimestamp().isValid()) {
    const QString& strTime = IUnit::datetime2string(wpt.getTimestamp(), IUnit::eTimeFormatLong,
                                                    QPointF(pos.x() * DEG_TO_RAD, pos.y() * DEG_TO_RAD));
...
void CDetailsWpt::slotNameChanged(const QString& name) { setWindowTitle(name); }
