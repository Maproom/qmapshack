...
CPlot::CPlot(CGisItemTrk* trk, CLimit& limit, CPlotData::axistype_e type, const QString& xLabel, const QString& yLabel,
             qreal factor, fTrkPtGetVal getX, fTrkPtGetVal getY, QWidget* parent)
    : IPlot(trk, type, eModeNormal, parent), limit(limit), factor(factor), getX(getX), getY(getY) {
...
CPlot::CPlot(CGisItemTrk* trk, CLimit& limit, IPlot::mode_e mode, QWidget* parent)
    : IPlot(trk, CPlotData::eAxisLinear, mode, parent), limit(limit) {
  connect(&limit, &CLimit::sigChanged, this, &CPlot::setLimits);
}
...
void CPlot::updateData() {
  ...
  for (const CTrackData::trkpt_t& trkpt : t) {
    if (!trkpt.isHidden() && getY(trkpt) != NOFLOAT) {
