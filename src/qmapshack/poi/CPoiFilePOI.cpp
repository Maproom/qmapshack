...
bool CPoiFilePOI::findPoiCloseBy(const QPoint& px, QSet<const IPoiItem*>& poiItems, QList<QPointF>& posPoiHighlight) const
{
...
void CPoiFilePOI::findPoisIn(const QRectF& degRect, QSet<const IPoiItem*>& pois, QList<QPointF>& posPoiHighlight)
{
    CTryMutexLocker lock(mutex);
    if(!lock.try_lock())
    {
