...
public:
    ...
    bool findPoiCloseBy(const QPoint& px, QSet<const IPoiItem*>& poiItems, QList<QPointF>& posPoiHighlight) const override;
    ///The POIs can be clustered together, so the icon is not necessarily displayed where the POI is.
    /// Thus the location where to draw the highlight is separately given
    void findPoisIn(const QRectF& degRect, QSet<const IPoiItem*>& pois, QList<QPointF>& posPoiHighlight) override;
