...
bool IUnit::strToDeg(const QString& str, qreal& lon, qreal& lat) {
  ...
  if (fabs(lon) > 180.0 || fabs(lat) > 90.0) {
    QMessageBox::warning(CMainWindow::getBestWidgetForParent(), tr("Error"), tr("Position values out of bounds. "),
                         QMessageBox::Ok, QMessageBox::NoButton);
    return false;
  }
...
QMap<QString, qreal> IUnit::timeToMKSMap = {
    {"s", 1.0}, {"min", 60.0}, {"h", 3600.0}, {"с", 1.0}, {"мин", 60.0}, {"ч", 3600.0},
};

QMap<QString, qreal> IUnit::distanceToMKSMap = {
    {"m", 1.0}, {"km", 1000.0}, {"mi", 1.0 / CUnitImperial::milePerMeter}, {"ft", 1.0 / CUnitImperial::footPerMeter},
    {"м", 1.0}, {"км", 1000.0},
};
