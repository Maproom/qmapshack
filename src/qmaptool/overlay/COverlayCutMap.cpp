...
void COverlayCutMap::slotSaveShape()
{
    ...
    QString filename = QFileDialog::getSaveFileName(&CMainWindow::self(), tr("Save shape..."), path + "/" + shapeFilename, "Shape file (*.shp)");
    if(filename.isEmpty())
    {
        return;
    }
...
void COverlayCutMap::slotLoadShape()
{
    ...
    QString filename = QFileDialog::getOpenFileName(&CMainWindow::self(), tr("LOad shape..."), path, "Shape file (*.shp)");
    if(filename.isEmpty())
    {
        return;
    }
