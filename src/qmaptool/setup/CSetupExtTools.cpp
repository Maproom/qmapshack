...
#define slot1(name, method)                                    \
  std::bind(&CSetupExtTools::slotSetPathXOverride, this, name, \
            [](const QString& path) { IAppSetup::self().method(path); })

#define slot2(method) std::bind(&CSetupExtTools::slotResetPathXOverride, this, []() { IAppSetup::self().method(); })

CSetupExtTools::CSetupExtTools(QWidget* parent) : QDialog(parent) {
  setupUi(this);
  setupGui();
