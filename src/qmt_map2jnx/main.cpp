...
struct jnx_hdr_t {
  jnx_hdr_t()
      : version(0x00000004), devid(0), expire(0), productId(0), crc(0), signature(0), signature_offset(0), zorder(25) {}
...
struct file_t {
  ...
  bool operator<(const file_t& other) const { return xscale > other.xscale; }
...
bool readTile(uint32_t xoff, uint32_t yoff, uint32_t xsize, uint32_t ysize, file_t& file, uint32_t* output) {
  ...
  if (rasterBandCount == 1) {
    ...
    if (pBand->RasterIO(GF_Read, (int)xoff, (int)yoff, xsize, ysize, tileBuf8Bit, xsize, ysize, GDT_Byte, 0, 0) ==
        CE_Failure) {
      return false;
    }
...
static void term_destination(j_compress_ptr cinfo) { jpgbuf.resize(jpgbuf.size() - cinfo->dest->free_in_buffer); }

static uint32_t writeTile(uint32_t xsize, uint32_t ysize, uint32_t* raw_image, FILE* fid, int quality,
                          int subsampling) {
  uint32_t size = 0;
...
static uint32_t scale2jnx(double scale) {
  ...
  return (uint32_t)floor(
      0.5 + 76437 * exp(log(2.000032708011) * floor(0.5 + log(scale * 10 * 130.2084 / 76437) / log(2.000032708011))));
}
...
int main(int argc, char** argv) {
  ...
  if (argc < 2) {
    fprintf(stderr,
            "\nusage: qmt_map2jnx -q <1..100> -s <411|422|444> -p <0..> -c \"copyright notice\" -m \"BirdsEye\" -n "
            "\"Unknown\" -x file1_scale,file2_scale,...,fileN_scale <file1> <file2> ... <fileN> <outputfile>\n");
    fprintf(stderr, "\n");
  ...
  printf("\n                                   %08X %08X, %08X %08X", jnx_hdr.left, jnx_hdr.top, jnx_hdr.right,
         jnx_hdr.bottom);
  ...
  for (int i = 0; i < nLevels; i++) {
    ...
    printf("\n    Level %i: % 5i tiles, offset %08X, scale: %i, %ix%i", i, level.nTiles, level.offset, level.scale,
